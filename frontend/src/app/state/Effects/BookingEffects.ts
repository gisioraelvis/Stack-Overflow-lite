    ...
    return this.actions$.pipe(
      ...
      mergeMap(() => {
        return this.bookingService.getUserBooking().pipe(
          ...
          catchError((error) =>
            of(BookingsActions.getBookingFail({ error: error.message }))
          )
        );
      })
    );
    ...
    return this.actions$.pipe(
      ...
      concatMap((action) => {
        return this.bookingService.addBooking(action.newbooking).pipe(
          map((successResponse) => {
            return BookingsActions.addbookingSuccess({
              message: successResponse,
            });
          }),
          catchError((error) =>
            of(BookingsActions.addbookingFail({ error: error.message }))
          )
        );
      })
    );
    ...
    return this.actions$.pipe(
      ...
      concatMap((action) => {
        return this.bookingService
          .updateBooking(action.id, action.updatedBooking)
          .pipe(
            map((successresponse) => {
              return BookingsActions.updateBookingSuccess({
                booking: successresponse,
              });
            }),
            catchError((error) =>
              of(BookingsActions.addbookingFail({ error: error.message }))
    ...
    return this.actions$.pipe(
      ...
      concatMap((action) => {
        return this.bookingService.deleteBooking(action.id).pipe(
          map((successResponse) => {
            return BookingsActions.deleteBookingSuccess({
              message: successResponse,
            });
          }),
          catchError((error) =>
            of(BookingsActions.deleteBookingFail({ error: error.message }))
          )
        );
      })
    );
